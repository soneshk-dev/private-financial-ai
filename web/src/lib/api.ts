...
export type Band = { month?: number; age?: number; p10?: number; p50: number; p90?: number };
export type GoalProjection = Goal & { overrides: Record<string, number | string>; projection: { kind: 'save' | 'debt' | 'none'; reason?: string; return_pct?: number; vol_pct?: number; basis?: string; series?: Band[]; end?: { p10: number; p50: number; p90: number }; probability?: number; confidence_pct?: number; required_monthly?: number | null; on_track?: boolean; rate_pct?: number; monthly_payment?: number; payoff_months?: number | null; interest_per_month_now?: number } };
export type RetirementSettings = { current_age: number | null; retire_age: number; end_age: number; annual_spend: number | null; annual_contribution: number; other_income: number; other_income_age: number; include_classes: string[]; crypto_haircut_pct: number };
...
export const api = {
  ...
  projections: () => j<Projections>('/api/plan/projections'),
  savePlan: (patch: Record<string, unknown>) => j<any>('/api/plan/settings', { method: 'PUT', body: JSON.stringify(patch) }),
